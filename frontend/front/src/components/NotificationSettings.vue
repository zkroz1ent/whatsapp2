<template>
    <div class="container mx-auto p-4">
      ...
      <div v-for="commission in commissions" :key="commission.id" class="mb-4">
        <label class="inline-flex items-center">
          <input type="checkbox" :value="commission.id" v-model="notifications.commissions" class="form-checkbox h-5 w-5 text-green-600" />
          <span class="ml-2">{{ commission.name }}</span>
        </label>
      </div>
  
      <button @click="saveNotifications" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
        Enregistrer
      </button>
